...
def plot_alpha_group(fileName, Data, dpi_save,s, bandwidth,cmap,  round_dec, norm_zero_one):
    ...
    plotName = fileName + "/Plots"
    f = plotName + "/plot_alpha_group_auto_bandwidth_bool_%s" % ( bandwidth)
    fig.savefig(f + ".eps", dpi=dpi_save, format="eps")
    fig.savefig(f + ".png", dpi=dpi_save, format="png")
...
def live_animate_culture_network_and_weighting(
    fileName: str,
    Data,
    layout: str,
    ...
    def update(i, Data, ax, cmap_culture, layout, title):
    ...
    ani = animation.FuncAnimation(
        ...
        fargs=(Data, ax, cmap_culture, layout, title),
        repeat_delay=500,
        interval=interval,
    )
    ...
    if save_bool:
        ...
        f = (
            animateName
            + "/live_animate_culture_network_and_weighting.mp4"
        )
        # print("f", f)
