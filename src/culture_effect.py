...
from run import parallel_run,live_run
...
if __name__ == "__main__":
        ...
        property_varied = "phi_list"#"alpha_change"#"culture_momentum"#"confirmation_bias"#"inverse_homophily" #MAKE SURE ITS TYPES CORRECTLY


        fileName = "results/%s_variation_%s_%s_%s_%s" % (property_varied,str(params["N"]),str(params["time_steps_max"]),str(params["K"]), str(reps))
        print("fileName: ", fileName)

        #No Culture
        phi_list_no_culture = [0,0,0]
        #Complete Culture
        phi_list_complete_culture = [1,1,1]
        #Baggaged Culutre
        phi_list_baggaged_culture = [0,0,1]
        property_varied_title = "Identity Presence"

        property_values_list = np.asarray([phi_list_no_culture,phi_list_complete_culture,phi_list_baggaged_culture])
        print("property_values_list",property_values_list)
        title_list = ["No identity", "Full Identity", "Slowed Identity"]

        params_list = produce_param_list(params,property_values_list, property_varied)

        data = parallel_run(params_list)#live_run(params_list)##better if a Multiple of 4
