...
from plot import (
    ...
    print_culture_time_series_two_properties,
)
...
if __name__ == "__main__":
        ...
        fileName = "results/alpha_attract_and_beta_threshold_variation_%s_%s_%s_%s" % (str(params["N"]),str(params["time_steps_max"]),str(params["K"]), str(reps))
        print("fileName: ", fileName)

        nrows = reps
        ncols = 3

        #alpha_attract_list = np.linspace(confirmation_bias_min,confirmation_bias_max, reps)
        alpha_attract_list = [0.1,1,10]
        beta_attract_list = [0.1,1,10]
        
        title_list = []
        for i in range(len(alpha_attract_list)):
            for v in range(len(beta_attract_list)):
                title_list.append(("Alpha attitude = %s, Beta attitude = %s") % (str(alpha_attract_list[i]), str(beta_attract_list[v])))

        print(title_list)

        data = []
        for i in alpha_attract_list:
            params["alpha_attract"] = i
            data_row = []
            for i in beta_attract_list:
                #no change in attention
                params["beta_attract"] = i
                data_row.append(generate_data(params))
            data.append(data_row)

        createFolderSA(fileName)

        print_culture_time_series_two_properties(fileName, data, alpha_attract_list, beta_attract_list,"Alpha attitude", "Beta attitude", nrows, ncols, dpi_save, round_dec)
        print_culture_time_series_clusters_two_properties(fileName, data, alpha_attract_list, beta_attract_list,"Alpha attitude", "Beta attitude", min_k,max_k,size_points, alpha_val, min_culture_distance,"Euclid", nrows, ncols, dpi_save, round_dec)
