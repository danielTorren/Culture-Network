...
from package.resources.plot import (
    bifurcation_plot_identity_or_not,
)
...
def main(
    ...
    results_identity_lists_identity = load_object(fileName + "/Data", "results_identity_lists_identity")
    results_identity_lists_no_identity = load_object(fileName + "/Data", "results_identity_lists_no_identity")
    ...
    if bifurcation_plot_data_analysis:#If you want to perform the cluster analysis for different bandwidths
        ...
        cluster_pos_matrix_list_identity = [calc_pos_clusters_set_bandwidth(np.asarray(results_identity_lists_identity[i]),identity_space,bandwidth) for i in range(len(property_values_list))] 
        cluster_pos_matrix_list_no_identity = [calc_pos_clusters_set_bandwidth(np.asarray(results_identity_lists_no_identity[i]),identity_space,bandwidth) for i in range(len(property_values_list))] 
    ...
    bifurcation_plot_identity_or_not(fileName,cluster_pos_matrix_list_identity,cluster_pos_matrix_list_no_identity,property_values_list, dpi_save, latex_bool = latex_bool)
